...
describe Options::Compatible do
  ...
  describe '#form_fields' do
    it 'returns fields for setting the option' do
      ...
      expect(subject.form_fields(form_builder)).to eql '<div class="radio"><label for="test_model_compatible_true"><input id="test_model_compatible_true" name="test_model[compatible]" type="radio" value="true" />On</label><div class="radio"><label for="test_model_compatible_false"><input checked="checked" id="test_model_compatible_false" name="test_model[compatible]" type="radio" value="false" />Off (recommended)</label></div></div>'
    end
  end
end
